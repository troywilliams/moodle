...
/// Start output log
$timenow  = time();
$logger =& logger_get_logger('Turnitin', 'Process events queue');
$logger->info("Server Time: ".date('r',$timenow));

$processed = plagiarism_turnitin_process_events_cron('mod_created');
$logger->info('mod_created, processed:'.$processed);

$processed = plagiarism_turnitin_process_events_cron('mod_updated');
$logger->info('mod_updated, processed:'.$processed);

$processed = plagiarism_turnitin_process_events_cron('mod_deleted');
$logger->info('mod_deleted, processed:'.$processed);

$processed = plagiarism_turnitin_process_events_cron('assessable_file_uploaded');
$logger->info('assessable_file_uploaded, processed:'.$processed);

$processed = plagiarism_turnitin_process_events_cron('assessable_files_done');
$logger->info('assessable_files_done, processed:'.$processed);
   
$difftime = microtime_diff($starttime, microtime());
$logger->info("Execution took ".$difftime." seconds");
