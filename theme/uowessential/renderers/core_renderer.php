...
/**
 * uowessential theme with the underlying Bootstrap theme.
 *
 * @package    theme
 * @subpackage uowessential
 ...
 */
 ...
 class theme_uowessential_core_renderer extends theme_bootstrapbase_core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = uowessential_performance_output($perf);
            }
        }
    ...
    protected function render_custom_menu(custom_menu $menu) {
    	...
    	*/
        ...
        if (isloggedin() && $hasdisplaymycourses) {
            ...
            if ($mycoursetitle == 'module') {
				$branchlabel = '<i class="icon-briefcase"></i>'.get_string('mymodules', 'theme_uowessential');
				$branchtitle = get_string('mymodules', 'theme_uowessential');
			} else if ($mycoursetitle == 'unit') {
				$branchlabel = '<i class="icon-briefcase"></i>'.get_string('myunits', 'theme_uowessential');
				$branchtitle = get_string('myunits', 'theme_uowessential');
			} else if ($mycoursetitle == 'class') {
				$branchlabel = '<i class="icon-briefcase"></i>'.get_string('myclasses', 'theme_uowessential');
				$branchtitle = get_string('myclasses', 'theme_uowessential');
			} else {
				$branchlabel = '<i class="icon-briefcase"></i>'.get_string('mycourses', 'theme_uowessential');
				$branchtitle = get_string('mycourses', 'theme_uowessential');
			}
            ...
            $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
 			if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
 				...
 				$branch->add('<em>'.get_string('noenrolments', 'theme_uowessential').'</em>',new moodle_url('/'),get_string('noenrolments', 'theme_uowessential'));
 			}
    	...
    	*/
        ...
        if (isloggedin() && $hasdisplaymydashboard) {
            $branchlabel = '<i class="icon-dashboard"></i>'.get_string('mydashboard', 'theme_uowessential');
            $branchurl   = new moodle_url('/my/index.php');
            $branchtitle = get_string('mydashboard', 'theme_uowessential');
            $branchsort  = 10000;
    ...
    */
    ...
    public function uowessentialblocks($region, $classes = array(), $tag = 'aside') {
